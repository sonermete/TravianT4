...
<img src="img/x.gif" class="r4Big" alt="r4Big"></button>
 ...
 ?>
<div class="clear"></div>
<h4 class="spacer">پیشنهاد های موجود در بازار</h4>

<table id="range" cellpadding="1" cellspacing="1">
	<thead>
		<tr>
			<th>پیشنهاد شده به من</th>
			<th><img src="img/x.gif" class="ratio" alt="نسبت"></th>
			<th>درخواست شده از من</th>
			<th>بازیکن</th>
			<th>مدت زمان</th>
			<th>عمل</th>
		</tr>
...
switch($market->onsale[$i]['gtype']) {
    ...
    echo "<td class=\"act none\">منابع کافی نیست.</td></tr>";
    }
    else if($market->merchantAvail() == 0 && $reqMerc <= $market->merchantAvail()) {
    echo "<td class=\"act none\">تاجر کافی نیست.</td></tr>";
    }
    else {
    echo "<td class=\"act\">
...
</div><div class=\"button-contents\">قبول پیشنهاد</div></div></button></td>";
    }
    echo"</tr>";
    }
}
...
echo "<tr><td class=\"none\" colspan=\"6\"><center>پیشنهادی در بازار موجود نیست.</center></td></tr>";
}
?>
</tbody></table>
<tr><td colspan="5"><p>
...
if(!isset($_GET['u']) && count($market->onsale) < 40) {
    echo "<span class=\"none\"><b>&laquo;</b></span> صفحات <span class=\"none\"><b>&raquo;</b></span>";
    }
    else if (!isset($_GET['u']) && count($market->onsale) > 40) {
    echo "<span class=\"none\"><b>&laquo;</b></span> صفحات <a href=\"build.php?id=<?php echo $id; ?>&t=1&u=40\">&raquo;</a>";
    }
    else if(isset($_GET['u']) && count($market->onsale) > $_GET['u']) {
    	if(count($market->onsale) > ($_GET['u']+40) && $_GET['u']-40 < count($market->onsale) && $_GET['u'] != 0) {
         echo "<a href=\"build.php?id=<?php echo $id; ?>&t=1&u=".($_GET['u']-40)."\">&laquo;</a> صفحات <a href=\"build.php?id=<?php echo $id; ?>&t=1&u=".($_GET['u']+40)."\">&raquo;</a>";
         }
         else if(count($market->onsale) > $_GET['u']+40) {
         	echo "<span class=\"none\"><b>&laquo;</b></span> صفحات <a href=\"build.php?id=<?php echo $id; ?>&t=1&u=".($_GET['u']+40)."\">&raquo;</a>";
         }
        else {
        echo "<a href=\"build.php?id=<?php echo $id; ?>&t=1&u=".($_GET['u']-40)."\">&laquo;</a> صفحات <span class=\"none\"><b>&raquo;</b></span>";
        }
    }
?>
</td></tr> 
